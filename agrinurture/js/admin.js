...
import { logoBoxMenuBarButton } from './accesiblefunctions.js';
    ...
    function UploadState(){
        ...
        // Show/Hide upload box on button click
        uploadButton.addEventListener('click', () => {
            const isVisible = uploadBox.style.display === 'block';
            uploadBox.style.display = isVisible ? 'none' : 'block';
            localStorage.setItem('uploadBoxVisible', !isVisible); // Save state
        });
    
        // Restore visibility state from localStorage
        document.addEventListener('DOMContentLoaded', () => {
            const uploadBoxVisible = localStorage.getItem('uploadBoxVisible') === 'true';
            if (uploadBoxVisible) {
